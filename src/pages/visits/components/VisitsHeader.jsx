...
export default function VisitsHeader({ navigate }) {
  return (
    <div className="relative w-full bg-[#1e3a8a] rounded-[2rem] p-6 text-white flex items-center justify-between overflow-hidden shadow-xl">
      ...
      <div className="flex items-center gap-5 relative z-10">
        <button onClick={() => navigate(-1)} className="bg-white/10 p-3 rounded-xl hover:bg-white/20 transition-all hover:scale-105 active:scale-95 group border-none outline-none cursor-pointer">
          <ChevronRight className="w-6 h-6 text-white group-hover:-translate-x-1 transition-transform" />
        </button>
        <div>
          <div className="flex items-center gap-3 mb-1"><h1 className="text-2xl font-bold">إدارة الزيارات</h1></div>
          <p className="text-blue-200 text-sm opacity-90 font-bold">إدارة مواعيد الرؤية والطلبات الخاصة بك</p>
        </div>
      </div>
